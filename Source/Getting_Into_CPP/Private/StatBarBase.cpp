...
void UStatBarBase::OnFloatStatUpdated(float OldValue, float NewValue, float MaxValue)
{
	...
	//just in case someone passes invalid value

	//Prevent divide by zero errors.
	//the macro SMALL_NUMBER world be appropriate here..
	//but KINDA_SMALL_NUMBER is the sort of name which goes against all the 'Stuffy' coding conventions,
	//and it's official EPIC Code , so I wanted to highlist that !;-)
	//I think its 'KINDA' amuing
...
void UStatBarBase::UpdateWidget()
{
	...
	PercentBar->SetVisibility(IsFullSize ? ESlateVisibility::Visible: ESlateVisibility::Collapsed);
}
